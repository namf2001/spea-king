...
export function usePronunciationAssessment() {
  ...
  const [results, setResults] = useState<PronunciationAssessmentResult | null>(null)
  ...
  const safelyExtractProperties = (result: any): Record<string, any> => {
    ...
    try {
      ...
      if (typeof result.properties.getProperty === "function") {
        ...
        if (result.properties.keys && typeof result.properties.keys === "function") {
          try {
      ...
      if (result.properties.entries && typeof result.properties.entries === "function") {
        try {
  ...
  const safelyExtractWords = (result: any, referenceText: string) => {
    try {
      ...
      if (result.json?.NBest?.[0]?.Words?.length > 0) {
        ...
        return result.json.NBest[0].Words.map((word: WordData) => {
          ...
          const prosodyFeedback = word.PronunciationAssessment?.Feedback?.Prosody
            ? {
                breakFeedback: {
                  errorTypes: word.PronunciationAssessment.Feedback.Prosody.Break?.ErrorTypes ?? [],
                  unexpectedBreak: word.PronunciationAssessment.Feedback.Prosody.Break?.UnexpectedBreak
                    ? { confidence: word.PronunciationAssessment.Feedback.Prosody.Break.UnexpectedBreak.Confidence ?? 0 }
                    : undefined,
                  missingBreak: word.PronunciationAssessment.Feedback.Prosody.Break?.MissingBreak
                    ? { confidence: word.PronunciationAssessment.Feedback.Prosody.Break.MissingBreak.Confidence ?? 0 }
                    : undefined,
                  breakLength: word.PronunciationAssessment.Feedback.Prosody.Break?.BreakLength ?? 0,
                },
                intonationFeedback: {
                  errorTypes: word.PronunciationAssessment.Feedback.Prosody.Intonation?.ErrorTypes ?? [],
                  monotone: word.PronunciationAssessment.Feedback.Prosody.Intonation?.Monotone
                    ? {
                        syllablePitchDeltaConfidence: 
                          word.PronunciationAssessment.Feedback.Prosody.Intonation.Monotone.SyllablePitchDeltaConfidence ?? 0,
                      }
                    : undefined,
                },
              }
      ...
      if (result.json?.DisplayText) {
        console.log("Trying to extract words from DisplayText:", result.json.DisplayText)

        // Split the recognized text into words
        const recognizedWords = result.json.DisplayText.split(/\s+/).filter(Boolean)

        // If we have pronunciation assessment results, try to match them with words
        if (recognizedWords.length > 0) {
          console.log("Created fallback words from DisplayText:", recognizedWords)
      ...
      if (referenceText) {
        console.log("Creating fallback words from reference text:", referenceText)
  ...
  const startRecognition = useCallback(
    async (referenceText: string) => {
      ...
      try {
        ...
        pronunciationAssessmentConfig.enableContentAssessmentWithTopic("general")
        ...
        newRecognizer.recognized = (s, e) => {
          console.log("Recognition event received:", JSON.stringify(e.result, null, 2))
          ...
          if (e.result.reason === SpeechSDK.ResultReason.RecognizedSpeech) {
            ...
            try {
              ...
              const extractedWords = extractWordsFromRawJson(rawJson, referenceText);
              ...
              const rawData = {
                ...
                pronunciationAssessment: {
                  ...
                  prosodyScore: rawJson?.NBest?.[0]?.PronunciationAssessment?.ProsodyScore,
                },
              }
              ...
              if (stopRequestedRef.current) {
                console.log("Results received after stop requested, closing recognizer")
                setTimeout(() => {
                  newRecognizer.close()
                  setRecognizer(null)
                  setIsProcessingResult(false)
        ...
        newRecognizer.canceled = (s, e) => {
          ...
          if (e.reason === SpeechSDK.CancellationReason.Error) {
            setError(new Error(`Speech recognition canceled: ${e.errorDetails}`))
          }
          setIsProcessingResult(false)
        }
    ...
    if (recognizer) {
      try {
        ...
        if (!resultReceivedRef.current) {
          ...
          if (!resultReceivedRef.current) {
            console.log("No results received after waiting, closing recognizer")
            recognizer.close()
            setRecognizer(null)
            setIsProcessingResult(false)

            // Create fallback results with reference text
            const referenceText = recognizer.properties.getProperty("ReferenceText", "")
            if (referenceText) {
              ...
              setError(new Error("No speech was detected. Please try again and speak clearly."))
            }
          }
        } else {
