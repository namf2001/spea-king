...
export default function ConversationTopicsContent(
  { topics, userId, error }: { topics: ConversationTopic[], userId: string, error?: string }
) {
    ...
    <>
      ...
      {!error && topics.length === 0 && (
        <Card className="mb-6 border-dashed">
          <CardContent className="pt-8 pb-6 flex flex-col items-center text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">No conversation topics yet</h3>
            <p className="text-muted-foreground mb-6 max-w-md">
              Create your first conversation topic to start practicing.
              AI will dynamically generate responses based on your chosen topic.
            </p>
            <Button
              ...
              size="lg"
            >
              <Plus className="h-4 w-4 mr-2" /> Create your first topic
            </Button>
          </CardContent>
        </Card>
      )}
          ...
          <div className="flex items-center justify-between mb-8 animate-slideInLeft">
            <div className="flex items-center gap-3">
              ...
              >
                ...
                />
                <MessageSquare className="h-5 w-5 sm:h-6 sm:w-6 text-white relative z-10" />
              </motion.div>
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold">Conversation Topics</h1>
            </div>
            <motion.div
                  ...
                  <Card className="overflow-hidden border hover:border-primary/50 transition-all">
                    <CardHeader>
                      <div className="flex justify-between items-start">
                        <CardTitle className="font-bold">{topic.title}</CardTitle>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>
              {isEditMode ? 'Edit Conversation Topic' : 'Add New Conversation Topic'}
            </DialogTitle>
            <DialogDescription>
              {isEditMode 
