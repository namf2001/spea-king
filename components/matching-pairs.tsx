...
interface MatchingPairsProps {
  ...
  readonly onComplete: (score: number, timeSpent: number, attempts: number) => void;
...
export function MatchingPairs({ pairs, onComplete, className }: MatchingPairsProps) {
  ...
  const [visibleVietnameseWords, setVisibleVietnameseWords] = useState<PairItem[]>([]);
  ...
  useEffect(() => {
    ...
    const vietnameseWords = pairs.map(p => p.vietnamese).sort(() => Math.random() - 0.5);
  ...
  const addNewWordsToVisible = (matchedEnglishId: string, matchedVietnameseId: string) => {
    ...
    if (nextEnglishWord) {
      ...
      setVisibleEnglishWords(prev => prev.filter(word => word.id !== matchedEnglishId));
    }
    ...
    if (nextVietnameseWord) {
      ...
      // Nếu không có từ mới, loại bỏ từ đã match
      setVisibleVietnameseWords(prev => prev.filter(word => word.id !== matchedVietnameseId));
    }
  };
  ...
  const handleItemClick = (item: PairItem) => {
    ...
    if (!selectedItem) {
      ...
      if (selectedItem.matchId === item.id || item.matchId === selectedItem.id) {
        // Match found
        const newMatchedPairs = new Set([...matchedPairs, selectedItem.id, item.id]);
        ...
        if (newLives <= 0) {
          setGameOver(true);
          setTimeout(() => {
            onComplete(0, Math.floor((Date.now() - startTime) / 1000), attempts + 1);
          }, 1500);
        }
    ...
    <div className={cn("flex flex-col gap-6", className)}>
      <div className="flex items-center gap-4">
        ...
        <div className="relative flex-1">
          <div className="h-3.5 rounded-full bg-gray-200 w-full">
            <div
          ...
          <div className="absolute top-0 left-0 w-full flex justify-between items-center">
            {[0, 50, 100].map((checkpoint, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`w-8 h-8 rounded-full flex items-center justify-center -mt-2.5 transition-all duration-300 ${isCompleted ? 'bg-[#4755FB] text-white' : 'bg-gray-200'
                    }`}
                >
                  {isCompleted && (
          ...
          <div className="absolute -right-2 -top-2.5">
            <div className="w-12 h-8 rounded-lg bg-white border-2 border-[#4755FB] flex items-center justify-center text-sm font-medium text-[#4755FB]">
            ...
            {visibleEnglishWords.map((item) => (
              <motion.button
                ...
                className={cn(
                  ...
                  "text-lg font-medium min-h-[60px] flex items-center justify-between",
                  getItemStyle(item)
                )}
      ...
      {gameOver && (
        <div className="absolute inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 text-center shadow-lg">
            ...
            <p className="text-gray-700 mb-4">Bạn đã hết mạng. Vui lòng thử lại.</p>
            <button
              onClick={() => {
                setGameOver(false);
                setAttempts(0);
                ...
                setVisibleVietnameseWords(allVietnameseWords.slice(0, BATCH_SIZE));
              }}
              className="px-4 py-2 bg-[#4755FB] text-white rounded-lg hover:bg-[#3748a1] transition-all duration-200"
            >
              Chơi lại
            </button>
          </div>
